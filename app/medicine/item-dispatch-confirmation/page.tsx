...
import { useGetDispatchedItemsQuery, ItemDispatch } from '@/store/services/drugApi';
import { printTable, formatServerDate } from '@/app/reports/reportUtils';
import styles from './ItemDispatchConfirmation.module.css';
import Link from 'next/link';

export default function ItemDispatchConfirmationPage() {
  ...
  const filterByDate = (list: ItemDispatch[]) => {
  ...
  const pending = filterByDate((dispatched as ItemDispatch[]).filter((d) => d.dispatchStatus === 'Pending'));
  const confirmed = filterByDate((dispatched as ItemDispatch[]).filter((d) => d.dispatchStatus === 'Confirmed'));
  const rejected = filterByDate((dispatched as ItemDispatch[]).filter((d) => d.dispatchStatus === 'Rejected'));

  const enrich = (list: ItemDispatch[]) => list.map(d => ({
    ...d,
    _itemName: (() => {
      const first = (d.items || [])[0]?.purchaseItem;
      return first?.item ? `${first.item.name} (${first.item.itemCode})` : (d.items || []).map(it => it.purchaseItem?.itemName || '').filter(Boolean).join(', ') || '—';
    })(),
  ...
  const columns = useMemo<SmartColumn<any>[]>(
    () => {
      const cols: SmartColumn<any>[] = [
        {
          header: 'Item Name', sortable: true,
          render: (row: any) => row._itemName || '—',
        },
        {
        ...
        },
        {
          ...
          render: (row) => (
            <span className={styles.actionBtns}>
              <Link href={`/medicine/item-dispatch-confirmation/${row.id}`} className={styles.viewLink}>
  ...
  return (
    <div className={styles.page}>
      <h2 className={styles.pageTitle}>{tab === 'waiting' ? 'Waiting Confirmation' : tab === 'confirmed' ? 'Confirmed Items' : 'Rejected Items'}</h2>
      ...
      <div className={styles.filterBar}>
        ...
        <div className={styles.filterActions}>
          ...
          <button className={`${styles.filterActionIcon} ${styles.filterPdf}`} onClick={() => {
            ...
            const headers = tab === 'waiting' ? ['Item Name', 'Dispatched By', 'Dispatched To', 'Items', 'Status', 'Dispatch Date']
              : tab === 'confirmed' ? ['Item Name', 'Dispatched By', 'Dispatched To', 'Items', 'Qty Confirmed', 'Status', 'Dispatch Date']
              : ['Item Name', 'Dispatched By', 'Dispatched To', 'Items', 'Qty Rejected', 'Status', 'Dispatch Date'];
            printTable('Item Dispatch', headers,
      ...
      <h2 className={styles.printTitle}>{tab === 'waiting' ? 'Waiting Confirmation' : tab === 'confirmed' ? 'Confirmed Items' : 'Rejected Items'}</h2>
